...
def _module_enabled(church):
    ...
    return bool(settings and settings.enable_music_module)
...
def index(request):
    ...
    if not _module_enabled(church):
        messages.error(request, "Music module is disabled in settings.")
        return redirect("dashboard")

    if not _can(request, "music.view"):
        return HttpResponseForbidden("You do not have access to the music module.")
